...
class MapReduceStepSpec extends WordSpec {
  ...
  "MapReduceSpec" should {
    "allow mainClass from a String" in {
      ...
      assert(ja.mainClass.contains(name))
    }

    "allow mainClass an instance" in {
      val ja = JarActivity(ec2).withMainClass(new SomeClass())
      assert(ja.mainClass.contains("com.krux.hyperion.activity.MapReduceStepSpec$SomeClass"))
    }

    "allow mainClass an object" in {
      val ja = JarActivity(ec2).withMainClass(SomeObject)
      assert(ja.mainClass.contains("com.krux.hyperion.activity.MapReduceStepSpec$SomeObject"))
    }
