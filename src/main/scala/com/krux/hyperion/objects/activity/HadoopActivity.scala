...
import com.krux.hyperion.objects.resource.EmrCluster

/**
 * Runs hadoop steps on an Amazon EMR cluster
 */
case class HadoopActivity private (
  id: PipelineObjectId,
  runsOn: EmrCluster,
  steps: Seq[HadoopStep],
  ...
  def withSteps(step: HadoopStep*) = this.copy(steps = steps ++ step)
