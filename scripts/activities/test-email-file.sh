...
function cleanUp() {
  rm -rf inputs output ${WORKING_DIR}
}

function setUp() {
  cleanUp
  mkdir -p inputs/{1,2,3,4,5} output/ ${WORKING_DIR}
...
INPUT1_STAGING_DIR=$(pwd)/inputs/5 ./email-file.sh ${COMPRESSED_FILENAME} ${ARGS} > test-logs/3.out 2> test-logs/3.err
[ $? -ne 1 ] && fail
echo "SUCCESS"

setUp "4. One compressed input, output compressed"
INPUT1_STAGING_DIR=$(pwd)/inputs/1 ./email-file.sh ${COMPRESSED_FILENAME} ${ARGS} > test-logs/4.out 2> test-logs/4.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv.gz ] && fail
#[ "$(head -1 test-logs/4.out)" != "'installing mailx'" ] && fail
#[ "$(tail -1 test-logs/4.out)" != "mail -v -a report.tsv.gz -r FROM -s SUBJECT -c CC TO" ] && fail
#[ $(gzcat ${WORKING_DIR}/report.tsv.gz | wc -l) -ne 3 ] && fail
echo "SUCCESS"

setUp "5. One uncompressed input, output compressed"
INPUT1_STAGING_DIR=$(pwd)/inputs/3 ./email-file.sh ${COMPRESSED_FILENAME} ${ARGS} > test-logs/5.out 2> test-logs/5.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv.gz ] && fail
...
INPUT1_STAGING_DIR=$(pwd)/inputs/1 INPUT2_STAGING_DIR=$(pwd)/inputs/2 ./email-file.sh ${COMPRESSED_FILENAME} ${ARGS} > test-logs/6.out 2> test-logs/6.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv.gz ] && fail
[ ! -f ${WORKING_DIR}/report\(1\).tsv.gz ] && fail
...
[ $(gzcat ${WORKING_DIR}/report.tsv.gz | wc -l) -ne 3 ] && fail
[ $(gzcat ${WORKING_DIR}/report\(1\).tsv.gz | wc -l) -ne 3 ] && fail
echo "SUCCESS"

setUp "7. Two uncompressed inputs, output compressed"
INPUT1_STAGING_DIR=$(pwd)/inputs/3 INPUT2_STAGING_DIR=$(pwd)/inputs/4 ./email-file.sh ${COMPRESSED_FILENAME} ${ARGS} > test-logs/7.out 2> test-logs/7.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv.gz ] && fail
[ ! -f ${WORKING_DIR}/report\(1\).tsv.gz ] && fail
...
INPUT1_STAGING_DIR=$(pwd)/inputs/2 INPUT2_STAGING_DIR=$(pwd)/inputs/3 ./email-file.sh ${COMPRESSED_FILENAME} ${ARGS} > test-logs/8.out 2> test-logs/8.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv.gz ] && fail
[ ! -f ${WORKING_DIR}/report\(1\).tsv.gz ] && fail
...
INPUT1_STAGING_DIR=$(pwd)/inputs/1 ./email-file.sh ${UNCOMPRESSED_FILENAME} ${ARGS} > test-logs/9.out 2> test-logs/9.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv ] && fail
[ "$(head -1 test-logs/9.out)" != "'installing mailx'" ] && fail
[ "$(tail -1 test-logs/9.out)" != "mail -v -a report.tsv -r FROM -s SUBJECT -c CC TO" ] && fail
[ $(cat ${WORKING_DIR}/report.tsv | wc -l) -ne 3 ] && fail
echo "SUCCESS"

setUp "10. One uncompressed input, output uncompressed"
INPUT1_STAGING_DIR=$(pwd)/inputs/3 ./email-file.sh ${UNCOMPRESSED_FILENAME} ${ARGS} > test-logs/10.out 2> test-logs/10.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv ] && fail
...
INPUT1_STAGING_DIR=$(pwd)/inputs/1 INPUT2_STAGING_DIR=$(pwd)/inputs/2 ./email-file.sh ${UNCOMPRESSED_FILENAME} ${ARGS} > test-logs/11.out 2> test-logs/11.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv ] && fail
[ ! -f ${WORKING_DIR}/report\(1\).tsv ] && fail
...
[ $(cat ${WORKING_DIR}/report.tsv | wc -l) -ne 3 ] && fail
[ $(cat ${WORKING_DIR}/report\(1\).tsv | wc -l) -ne 3 ] && fail
echo "SUCCESS"

setUp "12. Two uncompressed inputs, output uncompressed"
INPUT1_STAGING_DIR=$(pwd)/inputs/3 INPUT2_STAGING_DIR=$(pwd)/inputs/4 ./email-file.sh ${UNCOMPRESSED_FILENAME} ${ARGS} > test-logs/12.out 2> test-logs/12.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv ] && fail
[ ! -f ${WORKING_DIR}/report\(1\).tsv ] && fail
...
INPUT1_STAGING_DIR=$(pwd)/inputs/2 INPUT2_STAGING_DIR=$(pwd)/inputs/3 ./email-file.sh ${UNCOMPRESSED_FILENAME} ${ARGS} > test-logs/13.out 2> test-logs/13.err
[ $? -gt 0 ] && fail
[ ! -f ${WORKING_DIR}/report.tsv ] && fail
[ ! -f ${WORKING_DIR}/report\(1\).tsv ] && fail
