...
class HyperionContext(config: Config) {
  ...
  lazy val ec2TerminateAfter = Try(config.getString("hyperion.aws.ec2.terminate")).toOption.map(DpPeriod(_))
  ...
  lazy val emrTerminateAfter = Try(config.getString("hyperion.aws.emr.terminate")).toOption.map(DpPeriod(_))
