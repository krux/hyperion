...
case class Ec2Resource private (
  ...
  lazy val serialize = AdpEc2Resource(
    ...
    securityGroups = if (subnetId.nonEmpty) securityGroups.map(_.serialize) else None,
    securityGroupIds = if (subnetId.isEmpty) securityGroupIds.map(_.serialize) else None,
